//compile with: g++ Vicsek4D.cpp nrutil.cpp random_mars.cpp -o MD -O2 -w
...
int main (void) {
  ...
  Neq = 1000; //8000; //number of timesteps to equilibrate = 2000
  Nsim = 10000;
  freq = 50;
  //den = 2;
  ...
  rc = 1;
  L = pow(N/den, 1/4.);//pow(N/den,1./3.); //box length
  Ncell = (int) L/rc;
  Nmax = 1000;
  ...
  cell_list = imatrix(0,(Ncell*Ncell*Ncell*Ncell)-1,0,Nmax-1);
...
void lattice_packing() {
  int K = (int) (pow(N,1./4.)); //number of particles per row
  if (pow(K,4)<N){ K++;} //correct K if not all the particles will fit
...
void random_velocities() {
  ...
  for (int i=0; i<N; i++) {
    ...
    beta[i] = (random_mars->uniform() - 0.5)*PI;
    vx[i] = v*cos(theta[i])*sin(alpha[i])*sin(beta[i]);
    vy[i] = v*sin(theta[i])*sin(alpha[i])*sin(beta[i]);
    vz[i] = v*cos(alpha[i])*sin(beta[i]);
    vw[i] = v*cos(beta[i]);
  }
...
double calculate_potential() {
  ...
  for (int i=0; i<N; i++) {
    ...
    int cell_list_w = cell_list_indexi % Ncell;
    int cell_list_z  = ((cell_list_indexi - cell_list_w)/Ncell) % Ncell;
    int cell_list_y  = ((cell_list_indexi - cell_list_w - cell_list_z*Ncell)\
      /(Ncell*Ncell)) % Ncell;
    int cell_list_x  = ((cell_list_indexi - cell_list_w - cell_list_z*Ncell - cell_list_y*Ncell*Ncell)\
    ...
    for (int a=cell_list_x - 1; a<= cell_list_x + 1; a++) {
      for (int b=cell_list_y - 1; b<= cell_list_y + 1; b++) {
        for (int c=cell_list_z - 1; c<= cell_list_z + 1; c++) {
            for (int d = cell_list_w - 1; d<=cell_list_w+1; d++){
          ...
          if (dloc < 0) dloc += Ncell;
          if (dloc >= Ncell) dloc -= Ncell;

          //loop through all particles in the cell
          for (int j=0; j< Nmax; j++) {
	    int jloc = cell_list[aloc*Ncell*Ncell*Ncell + bloc*Ncell*Ncell + cloc*Ncell + dloc][j];
	    if (jloc != -1 && jloc != i) {
              //calculate interaction between particles
              double dxloc = x[i] - x[jloc];
              double dyloc = y[i] - y[jloc];
              double dzloc = z[i] - z[jloc];
              double dwloc = w[i] - w[jloc];
	      pbc(dxloc, dyloc, dzloc, dwloc);
	      double r2 = dxloc*dxloc + dyloc*dyloc + dzloc*dzloc + dwloc*dwloc;
              if(r2<r_cut2){
                double r2i = 1.0/r2;
...
void update_angles() {
  ...
  cos_beta_new = dvector(0,N-1);
  sin_beta_new = dvector(0,N-1);
      ...
      for (int b=cell_list_y - 1; b<= cell_list_y + 1; b++) {
	    ...
	    if (jloc != -1 && jloc != i) {
	      ...
	      double r2 = dxloc*dxloc + dyloc*dyloc + dzloc*dzloc + dwloc*dwloc;
              ...
              sin_beta_new[i] += sin(beta[jloc]);
              cos_beta_new[i] += cos(beta[jloc]);
              }
            }
          }
         }
  ...
  for (int i=0; i<N; i++) {
    ...
    alpha[i]= atan(sin_alpha_new[i]/cos_alpha_new[i])+delta*(random_mars->uniform() -0.5)*sqrt(dt)*PI;
    beta[i]= atan(sin_beta_new[i]/cos_beta_new[i])+delta*(random_mars->uniform() -0.5)*sqrt(dt)*PI;
    vx[i] = v*cos(theta[i])*sin(alpha[i])*sin(beta[i]);
    vy[i] = v*sin(theta[i])*sin(alpha[i])*sin(beta[i]);
    vz[i] = v*cos(alpha[i])*sin(beta[i]);
    vw[i] = v*cos(beta[i]);
  ...
  free_dvector(cos_beta_new,0,N-1);
  free_dvector(sin_beta_new,0,N-1);
	    ...
	    if (jloc != -1 && jloc != i) {
	      ...
	      double r2 = dxloc*dxloc + dyloc*dyloc + dzloc*dzloc + dwloc*dwloc;
              if(r2<r_cut2){
                ...
                fz[i] += fc * dzloc;
                fw[i] += fc * dwloc;
                virial += fc*dxloc*dxloc + fc*dyloc*dyloc + fc*dzloc*dzloc + fc*dwloc*dwloc;
              }
            }
          }
...
double calculate_kinetic(){
  double ke_sum = 0.0;
  for (int i=0; i<N; i++) {
    ke_sum += vx[i]*vx[i]+vy[i]*vy[i]+vz[i]*vz[i] + vw[i]*vw[i];
  }
...
void print_gr(int step){
  ...
  for (int i=0; i<N; i++) {
	...
	double dr = sqrt(dxloc*dxloc + dyloc*dyloc + dzloc*dzloc + dwloc*dwloc);
...
void print_positions(int i){
  ...
  for (int i=0; i<N; i++) {
    ...
    double zloc = z[i];
    double wloc = w[i];
    pbc(xloc, yloc, zloc, wloc);
    fprintf(out,"%f %f %f %f %f %f\n",xloc,yloc,zloc, wloc, vx[i],vy[i],vz[i], vw[i]);
  }
...
void create_cell_lists(){
    ...
    for (int b=0; b<Ncell; b++) {
      for (int c=0; c<Ncell; c++) {
        for (int d=0; d<Ncell; d++) {
           for (int j=0; j< Nmax; j++) {
	  cell_list[a*Ncell*Ncell*Ncell + b*Ncell*Ncell + c*Ncell + d][j] = -1;
          }
        }
      }
    }
  ...
  for (int i=0; i<N; i++) {
    ...
    if(wint> (Ncell-1)) wint=Ncell-1;

    cell_list_index[i] = xint * Ncell*Ncell*Ncell + yint*Ncell*Ncell + zint*Ncell + wint;


    bool particle_stored = false;
    for (int j=0; j< Nmax; j++) {
      if (cell_list[xint * Ncell*Ncell*Ncell + yint*Ncell*Ncell + zint*Ncell + wint][j] == -1) {
	cell_list[xint * Ncell*Ncell*Ncell + yint*Ncell*Ncell + zint*Ncell + wint][j] = i;
...
void pbc(double &x, double &y, double &z, double &w){
  if (x >= L/2.0) x -= L;
  if (x < -L/2.0) x += L;
  if (y >= L/2.0) y -= L;
  if (y < -L/2.0) y += L;
...
double calculate_msd()
{
  ...
  for (int i=0; i<N; i++)
	...
	(w_accum[i] - w_init[i])*(w_accum[i] - w_init[i]);
    }
  ...
  for (int i=0; i<N; i++)
    {
      ...
      dw = (w_accum[i] - w_init[i]);
      fourth += dx*dx*dx*dx+dy*dy*dy*dy+dz*dz*dz*dz + dw*dw*dw*dw;
...
double calculate_vel_auto()
{
  ...
  for (int i = 0; i<N; i++ )
    {
      corr += vx[i]*vx_init[i] + vy[i]*vy_init[i]+ vz[i]*vz_init[i] +vw[i]*vw_init[i];
    }
...
double calculate_order()
{
  ...
  m=sqrt(m_x*m_x+m_y*m_y + m_z*m_z + m_w*m_w)/v;
